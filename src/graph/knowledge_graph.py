...
Aligned with Pydantic v2 node/edge models from src.models.nodes.
"""
...
class KnowledgeGraph:
    ...
    Two graphs:
      - module_graph: DiGraph of modules and their import/call/configures relationships
      - lineage_graph: DiGraph of datasets and transformations
    All inserts go through Pydantic models — validation on write.
    """
    ...
    def add_module(self, node: ModuleNode) -> None:
        """Validate and insert a ModuleNode."""
    ...
    def add_import_edge(self, source: str, target: str, weight: int = 1) -> None:
        """source IMPORTS target. Validates via ImportEdge."""
        edge = ImportEdge(source=source, target=target, weight=weight)
        if edge.source in self.module_graph and edge.target in self.module_graph:
    ...
    def add_call_edge(
        ...
        """source CALLS target. Validates via CallsEdge."""
        edge = CallsEdge(
            source=source,
            target=target,
            caller=caller,
    ...
    def add_configures_edge(
        self, source: str, target: str, weight: int = 1
    ) -> None:
        """source CONFIGURES target. Validates via ConfiguresEdge."""
        edge = ConfiguresEdge(source=source, target=target, weight=weight)
        if edge.source in self.module_graph and edge.target in self.module_graph:
            self.module_graph.add_edge(
                edge.source,
                edge.target,
                edge_type=EdgeType.CONFIGURES.value,
                weight=edge.weight,
            )

    def add_function(self, fn: FunctionNode) -> None:
        """Register a FunctionNode in the function index."""
        self.function_index[fn.qualified_name] = fn

    # ── Lineage Graph ──────────────────────────────────────────────────────────

    def add_dataset(self, node: DatasetNode) -> None:
        """Validate and insert a DatasetNode."""
    ...
    def add_transformation(self, txn: TransformationNode) -> None:
        """
        Validate and insert a TransformationNode.
        Automatically creates CONSUMES edges (dataset→txn)
        and PRODUCES edges (txn→dataset).
        """
        ...
        for src in txn.source_datasets:
            ...
            consumes = ConsumesEdge(source=src, target=txn.id)
        ...
        for tgt in txn.target_datasets:
            ...
            produces = ProducesEdge(source=txn.id, target=tgt)
            self.lineage_graph.add_edge(
                produces.source,
                produces.target,
    ...
    def find_lineage_sources(self) -> List[str]:
        """Nodes with in_degree=0 in the lineage graph (raw data sources)."""
        return [
            n for n, d in self.lineage_graph.in_degree()
            if d == 0 and self.lineage_graph.nodes[n].get("node_type") == "dataset"
        ]

    def find_lineage_sinks(self) -> List[str]:
        """Nodes with out_degree=0 in the lineage graph (output datasets)."""
        return [
            n for n, d in self.lineage_graph.out_degree()
            if d == 0 and self.lineage_graph.nodes[n].get("node_type") == "dataset"
        ]
    ...
    def upstream_of(self, dataset_name: str) -> List[Tuple[str, str]]:
        ...
        return [
            (n, self.lineage_graph.nodes[n].get("node_type", "unknown"))
            for n in ancestors
        ]
    ...
    def save(self, output_dir: Path) -> None:
        """Serialize both graphs and function index to JSON."""
        output_dir.mkdir(parents=True, exist_ok=True)

        # Module graph — use node_link_data for full NetworkX serialization
        ...
        # Function index — serialize via Pydantic to_dict()
        ...
        if lg_path.exists():
            ...
            for node_id, attrs in kg.lineage_graph.nodes(data=True):
                ...
                if node_type == "dataset":
                    ...
                    try:
                        # line_range may be stored as list; coerce to tuple
                        raw = {k: v for k, v in attrs.items() if k != "node_type"}
                        if "line_range" in raw and isinstance(raw["line_range"], list):
                            raw["line_range"] = tuple(raw["line_range"])
                        kg.transformations[node_id] = TransformationNode(**raw)
                    except Exception:
                        pass

        if fn_path.exists():
            fn_data = json.loads(fn_path.read_text())
            for qname, attrs in fn_data.items():
                try:
                    kg.function_index[qname] = FunctionNode(**attrs)
                except Exception:
                    pass
