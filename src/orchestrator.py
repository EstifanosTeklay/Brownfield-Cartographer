"""
Orchestrator: wires Brownfield Cartographer agents in sequence.
Surveyor → Hydrologist → (optional) Semanticist → Archivist
...
def default_output_dir_for_repo(repo_path: Path) -> Path:
    """
    Default location for analysis artifacts for a given repo.
    We store all cartography outputs under:
      <project_root>/cartography/<repo_name>/
    instead of inside the analyzed repo itself.
    """
...
class Orchestrator:
    """
    Pipeline: Surveyor + Hydrologist + (optional) Semanticist + Archivist.
    ...
    def __init__(
        ...
        self.output_dir = (output_dir or default_output_dir_for_repo(self.repo_path)).resolve()
        self.verbose = verbose
        self.enable_semanticist = enable_semanticist
    ...
    def run_analysis(self) -> Dict[str, Any]:
        ...
        changed_files = []
        if self.force_full:
            self._log("Force full mode: running full pipeline regardless of incremental cache")
        ...
        try:
            ...
            if self.enable_semanticist:
                ...
                self._log("Skipping Agent 3: Semanticist (disabled)")
                report["agents"]["semanticist"] = {"status": "skipped", "reason": "disabled"}
                print()
    ...
    def load_existing(cls, repo_path: Path) -> "Orchestrator":
        ...
        output_dir = default_output_dir_for_repo(repo_path.resolve())
